    ...
    <div class="box-footer">
        <ul class="mailbox-attachments clearfix">
            @foreach($grid->rows() as $row)
                <li>
                    <span class="mailbox-attachment-icon has-img"><img src="{!! isset($server) ? $server : Admin::configs('upload.host') !!}/{!! $row->column($image_column) !!}" alt="Attachment"></span>
                        ...
                        <span class="mailbox-attachment-size">
                          <input type="checkbox" class="grid-item" data-id="{{ $row->id() }}" />
                            ...
                            @if($grid->allowActions())
                                {!! $row->actions() !!}
                                <a href="{!! isset($server) ? $server : Admin::configs('upload.host') !!}/{!! $row->column($image_column) !!}" target="_blank" download="custom-filename.jpg"><i class="fa fa-cloud-download"></i></a>
                            @endif
                            </span>
                        </span>
                    </div>
